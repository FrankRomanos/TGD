...
namespace TGD.UI
{
    ...
    /// ֵλ HUD
    /// ʾǰ/ + EnergyRegenPer2s
    /// УְҵԴԶɨְҵԴͣų HP/Energy
    /// ףغʱ䶹TurnTime Сͼ꣬ RemainingTime
    /// </summary>
    public sealed class TurnHudController : BaseTurnUiBehaviour
    {
        ...
        protected override void HandleRuntimeChanged(Unit u)
        {
        ...
        void RebuildStrip()
        {
            ...
            _segments.Clear();
            _typeSnapshot.Clear();            //  ؼ޸ʱտ

            if (_active == null) return;

            // 1) ɨְҵ  Դͼϣų HP/Energy
        ...
        List<CostResourceType> CollectTypesFromSkills(string classId)
        {
            ...
            list.Sort((a, b) => ((int)a).CompareTo((int)b)); // ȶ˳
            return list;
        }
