...
namespace TGD.HexBoard
{
    ...
    public sealed class HexOccServiceAdapter : MonoBehaviour, IOccupancyService
    {
        ...
        readonly Dictionary<OccToken, OccReserveMode> _tokenModes = new();
            ...
            OccReserveMode mode = OccReserveMode.SoftPath)
        {
            ...
            var reservedHard = new List<Hex>();
            var reservedSoft = new List<Hex>();
            ...
            switch (mode)
            {
                case OccReserveMode.EndOnlyHard:
                    if (unique.Count > 0)
                    {
                        ...
                        if (!store.TempReserve(last, actor))
                        {
                            result = OccReserveResult.Blocked;
                            return false;
                        }
                        reservedHard.Add(last);
                    }
                    break;
                ...
                case OccReserveMode.PathHard:
                    for (int i = 0; i < unique.Count; i++)
                    {
                        ...
                        {
                            reservedHard.Add(cell);
                            continue;
                        }

                        if (reservedHard.Count > 0)
                            store.TempRelease(actor, reservedHard);
                        result = OccReserveResult.Blocked;
                        return false;
                    }
                    break;
                ...
                default:
                    for (int i = 0; i < unique.Count; i++)
                    {
                        ...
                        {
                            reservedSoft.Add(cell);
                            continue;
                        }

                        if (reservedSoft.Count > 0)
                            store.TempReleaseSoft(actor, reservedSoft);
                        result = OccReserveResult.Blocked;
                        return false;
                    }
                    break;
            ...
            token = NewToken();
            var ledgerCells = new HashSet<Hex>();

            for (int i = 0; i < reservedHard.Count; i++)
                ledgerCells.Add(reservedHard[i]);

            for (int i = 0; i < reservedSoft.Count; i++)
                ledgerCells.Add(reservedSoft[i]);

            _tokenToCells[token] = ledgerCells;
            _tokenModes[token] = mode;
            ...
            OccDiagnostics.LogReserve(token, actor.Id, ledgerCells.Count, mode);
            return true;
        }
        ...
        public bool Commit(UnitRuntimeContext ctx, OccToken token, Hex finalAnchor, Facing4 facing, out OccTxnId txn, out OccFailReason reason)
        {
            ...
            _tokenModes.TryGetValue(token, out var mode);

            if (reservedCells != null && reservedCells.Count > 0)
            {
                store.TempRelease(actor, reservedCells);
                store.TempReleaseSoft(actor, reservedCells);
            }

            if (tokenList != null && tokenList.Count == 1)
            {
                store.TempClearForOwner(actor);
                store.TempClearSoftForOwner(actor);
            }
            ...
            if (ok)
            {
                ...
                _tokenModes.Remove(token);
                tokenList.Remove(token);
                if (tokenList.Count == 0)
                    _actorToTokens.Remove(actor);
            }
            else if (reservedCells != null && reservedCells.Count > 0)
            {
                switch (mode)
                {
                    case OccReserveMode.SoftPath:
                        foreach (var cell in reservedCells)
                            store.TempReserveSoft(cell, actor);
                        break;
                    case OccReserveMode.EndOnlyHard:
                    case OccReserveMode.PathHard:
                    default:
                        foreach (var cell in reservedCells)
                            store.TempReserve(cell, actor);
                        break;
                }
            }
        ...
        public bool Cancel(UnitRuntimeContext ctx, OccToken token, string reasonTag = null)
        {
            ...
            store.TempReleaseSoft(actor, cells);
            _tokenToCells.Remove(token);
            _tokenModes.Remove(token);
