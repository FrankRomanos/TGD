        ...
        public void Despawn(Unit unit)
        {
            ...
            if (turnManager != null)
            {
                if (record.context != null)
                    turnManager.UnregisterContext(record.context);

                var store = record.context != null ? record.context.cooldownHub?.secStore : null;
                if (store != null)
                    turnManager.UnregisterCooldownStore(store);
            }
        ...
        void RegisterTurnSystems(Unit unit, UnitRuntimeContext context, CooldownHubV2 hub, UnitFaction faction)
        {
            if (turnManager != null && unit != null && context != null)
            {
                ...
                if (hub != null && hub.secStore != null)
                    turnManager.RegisterCooldownStore(hub.secStore);
            }
