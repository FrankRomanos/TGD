...
namespace TGD.CoreV2
{
    public static class OccDiagnostics
    {
        ...
        public static void LogReserve(OccToken token, string actorId, int cellCount, OccReserveMode mode)
        {
            if (!TraceLog) return;
            Debug.Log($"[Occ] Reserve token={token} owner={actorId} cells={cellCount} mode={mode}");
        }
