...
IOcc 现在的“原子内核”
唯一写入通道： 所有占位变更只允许走 IOccupancyService（HexOccupancyService.Get() → HexOccupancy 只是读结构）。
核心 API（已用在 mover/attack）：
• bool TryPlace(UnitRuntimeContext ctx, Hex anchor, Facing4 face, out OccTxnId txn, out OccFailReason reason)
• bool TryMove(UnitRuntimeContext ctx, Hex anchor, Facing4 face, out OccTxnId txn, out OccFailReason reason)
• bool Commit(UnitRuntimeContext ctx, OccToken token, Hex finalAnchor, Facing4 finalFace, out OccTxnId txn, out OccFailReason reason)
• void Cancel(UnitRuntimeContext ctx, OccToken token, string why)
• bool ReservePath(UnitRuntimeContext ctx, List<Hex> path, out OccToken token, out OccReserveResult result, OccReserveMode mode)
• 读侧（在 HexOccupancy 上）：IsBlocked/IsBlockedFormal/TryGetActor/CellsOf/CanPlaceIgnoringTemp
原子语义：
1. 预览阶段只做软预留：ReservePath(..., SoftPath) → 得到 OccToken；
2. 确认阶段：优先 Commit(ctx, token, finalAnchor, finalFace, ...)；没有 token 就走 TryMove(...)（单步原子移动）；
3. 终止/退出：统一 Cancel(ctx, token, ...)（幂等，不会脏留）；
4. 只有 Commit 成功后才更新 Unit 与 UnitGridAdapter（通过 UnitRuntimeBindingUtil.SyncUnit(...)）；
5. 足迹/多格靠 IGridActor.Footprint + IOcc 的 Formal 占位判断完成，不再需要桥。
这套流程已经把“幽灵占位/双写”彻底消灭：UI/控制器只读 _occ 状态，写入只走 IOcc。
控制器接入范式（已在你现有 mover/attack 落地）
• 绑定：OccControllerReady.Ensure(ctx, this, out _selfActor, placeIfMissing:true)
• 读坐标：CurrentAnchor = _selfActor.Anchor；路径合法性用 `var pass = PassabilityFactory.ForMove(_occ, _selfActor, CurrentAnchor);`
• 预览：算路径 → ReservePath(...) → 保存 OccToken；
• 确认：移动/接近 → 补动画 → Commit(token, finalAnchor, finalFace, ...)；
• 放弃/Disable：Cancel(token, "reason")；
• 绝不再调用 Bridge.MoveCommit/EnsurePlacedNow/AnchorVersion 之类。以后再做的“第七/第八步”（等 Boss 技能时上）
不急，保持扩展位即可：
1. HardPath/TimedHold：在 ReservePath 增加硬预留/时限预留（群体冲锋/队形锁定）
2. 批量事务：多单位交换位（swap/push/pull），BeginTxn() → 多个 TryMove → CommitTxn()
3. 强制位移内核：规范化 Knockback/吸附：输入（起点、方向、优先规则）→ IOcc.ResolveDisplacement() → TryMove/Commit
4. 序列化/回放：IOcc Snapshot/Restore，用于切场缓存、录像回放、回滚
5. 占位校验器：帧末一致性扫描（开发期开）
6. 多层/楼层标记：给 Hex 增加 layer/floor 扩展位，IOcc 以“(cell,layer)”作为键
7. 动态障碍通道：陷阱、门、机关把可通行规则挂到 Passability 管线（不触 IOcc 写）
